...
def convert_depth_unit(val: str, to_metric: bool) -> str:
    ...
    # Cari pola angka diikuti oleh ' atau FT
...
def detect_anomalies(row: Dict) -> Dict:
    ...
    # Rule 1: Durasi sangat panjang (>12 jam) tapi tidak ada perubahan kedalaman
    ...
    # Rule 2: Cek data hasil swab (Water, Oil, Sediment) jika ada
    if 'WATER' in desc or 'OIL' in desc:
        # Cek angka persentase (sangat sederhana)
        percents = re.findall(r'(\d+)%', desc)
        if percents:
            for p in percents:
                if int(p) > 100:
                    anomalies.append("⚠️ Persentase > 100%")
                if int(p) < 0:
                    anomalies.append("⚠️ Persentase negatif")
...
def main():
    ...
    with st.sidebar:
        ...
        api_key_input = st.text_input("Kunci API Gemini:", type="password", value=st.session_state.api_key, placeholder="AIzaSy...")
        if api_key_input != st.session_state.api_key:
            st.session_state.api_key = api_key_input

        st.session_state.use_ai = st.checkbox("Gunakan AI untuk parsing", value=st.session_state.use_ai)
        st.session_state.use_metric = st.checkbox("Konversi ke Satuan Metrik (Meter)", value=st.session_state.use_metric)
        ...
        if st.button("🔄 Proses Data", type="primary", use_container_width=True):
            if raw_input.strip():
                with st.spinner("Memproses data..."):
                    ...
                    # Tambahkan Fase dan Anomali ke setiap baris
    ...
    with tab2:
        ...
        if st.session_state.processed_data:
            ...
            # Tampilkan dengan styling kondisi anomali
            st.dataframe(
                df, 
